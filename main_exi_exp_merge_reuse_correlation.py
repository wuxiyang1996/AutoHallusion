...
def run_exp(args, meta_log = "exp_results.json"):
    ...
    reuse_obj_scene_align = args["reuse_scene_obj_align"]
        ...
        while not completed:
            ...
            try:
                ...
                if reuse_scene:
                    ...
                    shutil.copyfile(path_header_replace(scene_info_loaded["path"], reuse_dataset_raw_data_path, scene_load_id), init_img_path)
                    ...
                    if gen_new_scene:
                        if scene_diversity:
                            ...
                            if scene_constrain is None:
                                scene_constrain = "Try to generate a different scene from the following: {}.".format(",".join(scene_lst))
                            else:
                                scene_constrain += "; and try to generate a different scene from the following: {}.".format(",".join(scene_lst))

                        scene_name = scene_thinking(constraint=scene_constrain, temperature=1.5)

                        # Retrieve words aligned with the scene image
                        if same:
                            word_list, _ = generate_noun_given_scene_aimodel(num=obj_count + 1, scene=scene_name, temperature=temp_generate_noun_given_scene)
                        else:
                            word_list, _ = generate_noun_given_scene_aimodel(num=obj_count, scene=scene_name, temperature=temp_generate_noun_given_scene)
                        ...
                        if same:
                            ...
                            if len(word_list) >= obj_count:
                                ...
                                # word_list, _ = generate_noun_given_scene_gpt4v(num=obj_count, scene=scene_name, temperature=0)
                                word_list, _ = generate_noun_given_scene_aimodel(num=obj_count, scene=scene_name, temperature=temp_generate_noun_given_scene)
                                generate_image_given_scene(word_list, scene_name, image_name=init_img_path)
                                gen_new_scene = True
                ...
                if reuse_obj:
                    ...
                    target_obj = obj_info_loaded["target_obj_name"]
                    disturbing_obj = obj_info_loaded["disturbing_obj_name"]

                    shutil.copyfile(path_header_replace(obj_info_loaded["path"], reuse_dataset_raw_data_path, load_id), os.path.join(save_loc, "obj.png"))
                    shutil.copyfile(path_header_replace(obj_info_loaded["pure_obj_path"], reuse_dataset_raw_data_path, load_id), os.path.join(save_loc, "pure_obj.png"))
                    shutil.copyfile(path_header_replace(obj_info_loaded["mask_path"], reuse_dataset_raw_data_path, load_id), os.path.join(save_loc, "mask_obj.png"))
                    ...
                    if obj_diversity:
                        objs_lst = [obj[0]["target_obj_name"] for obj in list(database_obj_ref.values())]
                        if len(objs_lst) > diversity_count:
                            objs_lst = random.sample(objs_lst, diversity_count)
                        if len(objs_lst) > 0:
                            diversity_cond = " Try to generate a different object from the following: {}.".format(",".join(objs_lst))
                        else:
                            diversity_cond = ""
                    else:
                        diversity_cond = ""
                    ...
                    target_obj_dict = random.choice(list(database_obj_ref.values()))
                    target_obj = target_obj_dict["target_obj_name"]
                    disturbing_obj = target_obj_dict["disturbing_obj_name"]

                    shutil.copyfile(target_obj_dict["path"], os.path.join(save_loc, "obj.png"))
                    shutil.copyfile(target_obj_dict["pure_obj_path"], os.path.join(save_loc, "pure_obj.png"))
                    shutil.copyfile(target_obj_dict["mask_path"], os.path.join(save_loc, "mask_obj.png"))
                ...
                logger.warning("[Target Model Generated] target object caption: " + ground_truth["irrelevant_object_description"])
                ...
                logger.warning("[Target Model Generated] Target-detected Object Distances: " + str(
                ...
                existence_results, exi_case_result = evaluation_existence_given_pred_model(result_img_path, ground_truth,
                                                                                result_caption, vqa_model_func=vqa_aimodel, logger=logger, debug=True)
                ...
                if gen_new_obj:
                    obj_key_char = list([val for val in target_obj if val.isalpha() or val.isnumeric()])
                    obj_key = "".join(obj_key_char).lower()

                    obj_details = {
                        "target_obj_name": target_obj,
                        "disturbing_obj_name": disturbing_obj,
                ...
                result[case_name]["target_obj_img"] = os.path.join(save_loc, "obj.png")
                result[case_name]["target_obj_mask"] = os.path.join(save_loc, "mask_obj.png")
                result[case_name]["existence_results"] = exi_case_result
                result[case_name]["disturbing_obj"] = disturbing_obj
                result[case_name]["correlation_results"] = corr_case_result
